...
- Before doing anything else, verify this is the `mobx-keystone` repository by checking:
...
2. Ask which package to release (`lib`, `yjs`, `loro`).
3. Resolve selected package paths (`package.json`, changelog, publish dir).
4. Read current version from selected package `package.json`.
5. Check for an `## Unreleased` section in the selected package changelog.
...
6. Parse the unreleased changelog bullets.
7. Propose `major`, `minor`, and `patch` next versions from current version.
8. Tell the user:
   - current version,
   - unreleased summary,
   - recommended bump with rationale,
   - all three selectable bump options.
9. Ask the user to select bump type.
10. After selection, compute target version and draft the exact edits to apply (do not write files yet):
...
11. Show planned diff summary and ask for confirmation to apply edits and execute pre-push steps.
12. If confirmed, run release commands in order:
   - Ensure working tree is clean or only has intended release files.
   - Switch to `master` and update it (`git fetch`, `git pull`).
...
13. Ask for final confirmation before push+publish.
14. If confirmed, finish release:
   - Push commit and tag to `origin master`.
   - Publish from selected package directory using `npm publish`.
15. Report exact outputs for commit SHA, tag, push status, and publish result.

## Command Template

```bash
# choose PACKAGE=lib|yjs|loro first
