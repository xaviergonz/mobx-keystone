...
This middleware observes actions in a given tree. It invokes listeners only for top-level actions, not for nested child actions or intermediate flow steps. Life-cycle hooks such as `onAttachedToRootStore` and its returned disposer are also excluded.
...
- `onStart` listeners are called before the action executes and allow cancellation by returning a new result (either a return or a throw).
...
The `ActionCall` passed to a listener is not directly serializable. Before storing or sending it, use `serializeActionCall`. Apply the serialized call with `applySerializedActionAndTrackNewModelIds` on the server or `applySerializedActionAndSyncNewModelIds` on clients, as shown in the [Client/Server Example](../examples/clientServer/clientServer.mdx).
