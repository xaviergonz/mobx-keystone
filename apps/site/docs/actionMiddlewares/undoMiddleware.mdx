...
| `undoQueue` | `ReadonlyArray<UndoEvent>` | The undo stack. The first operation to undo is the last one in the array. |
| `redoQueue` | `ReadonlyArray<UndoEvent>` | The redo stack. The first operation to redo is the last one in the array. |
| `undoLevels` | `number` | The number of undo actions available. |
| `canUndo` | `boolean` | Whether there is at least one action to undo. |
| `redoLevels` | `number` | The number of redo actions available. |
| `canRedo` | `boolean` | Whether there is at least one action to redo. |

And the following methods:

| Method | Description |
| --- | --- |
| `undo()` | Undoes the last action. Throws if there is no action to undo. |
| `redo()` | Redoes the previous action. Throws if there is no action to redo. |
| `clearUndo()` | Clears the undo queue. |
| `clearRedo()` | Clears the redo queue. |
| `withoutUndo(fn)` | Runs a synchronous block without recording its changes in this manager. |
| `withGroup(...)` / `withGroupFlow(...)` | Groups several operations into one undo step. |
| `createGroup(name?)` | Creates a group that can span separate synchronous blocks. |
| `dispose()` | Disposes of the undo middleware. |

## `UndoEvent`

Each undo step is stored as an `UndoEvent`. A single action records an `UndoSingleEvent` with:

- `type: UndoEventType.Single` - Identifies a single-action event.
...
A grouped step records an `UndoEventGroup` instead:

- `type: UndoEventType.Group` - Identifies a grouped event.
- `groupName?: string` - Optional name passed to the grouping API.
- `events: ReadonlyArray<UndoEventWithoutAttachedState>` - The single or nested grouped events in this step.

Both forms include `attachedState`, with optional `beforeEvent` and `afterEvent` values when the middleware is configured to save attached state. Treat events and queues as read-only; use the `UndoManager` methods to manipulate history. Events are plain objects rather than tree nodes, and attached states are kept as returned by `save()` (they are not copied), so `save()` should return a new value each time rather than one that is later mutated.

## Storing the undo store inside your models

`undoMiddleware` accepts an optional `UndoStore` as its second argument. When omitted, it creates an in-memory store owned by the undo manager. Pass an `UndoStore` from your model tree when the queues themselves need to be snapshotted or persisted.
...
The store keeps each event as [frozen data](../frozen.mdx), so in its snapshot every event is saved as `{ $frozen: true, data: event }`. Snapshots saved by earlier versions, where events were saved as plain objects, can still be loaded.

## Making some changes skip undo/redo

To exclude some changes from undo recording, use the manager's `withoutUndo` method or the global `withoutUndo` helper:
...
Another possibility is to use `createGroup` to group synchronous actions across separate code blocks:
