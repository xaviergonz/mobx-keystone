...
Both forms include `attachedState`, with optional `beforeEvent` and `afterEvent` values when the middleware is configured to save attached state. Treat events and queues as read-only; use the `UndoManager` methods to manipulate history.
