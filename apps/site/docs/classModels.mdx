...
- Class models, which combine observable data with local behavior (actions/views) and life-cycle hooks.
- Data models, which define behavior over plain, untagged data and have only a limited life-cycle hook.
- Plain objects, which store structured data or act as key-value maps without model behavior or hooks.
- Arrays.
...
Class models and data models provide `toString({ withData? })` for a string representation. Data is included by default, including when `withData` is `undefined`; pass `false` to omit it.

## Your first class model

A class model is a class decorated with `@model(...)` that extends `Model({ ... })`:

- The string passed to `@model` identifies the model type in snapshots, so it must be unique across your whole application.
- The properties passed to `Model` are the model data: observable, part of snapshots and patches, and required when creating an instance unless they have a default.
- Methods decorated with `@modelAction` are the only place where model data can change. Being actions also gives them middlewares, replication, and the other action features.
- Views are regular getters, usually decorated with MobX's `@computed`.

Properties can be declared with `prop` (TypeScript types only) or with `tProp` (TypeScript types plus runtime type checking):
...
To make a property truly optional, include `undefined` in its type: `x: prop<string | undefined>()`.
...
To make a property truly optional, use `types.maybe`: `x: tProp(types.maybe(types.string))`. See [runtime type checking](./runtimeTypeChecking.mdx) for all the available types.
...
A default is used whenever the property is `null` or `undefined` in the creation data or the input snapshot.

| Without runtime type checking | With runtime type checking | Default |
| --- | --- | --- |
| `prop<T>()` | `tProp(type)` | None. |
| `prop<T>(defaultValue)` | `tProp(type, defaultValue)` | `defaultValue`. Use it for primitives only. |
| `prop<T>(() => defaultValue)` | `tProp(type, () => defaultValue)` | The result of calling the function. Use it for objects, arrays, and models, so every instance gets its own copy. |

`tProp` also has two shorthands: `tProp("text")`, `tProp(0)`, and `tProp(false)` are the same as `tProp(types.string, "text")` and so on, and `tProp([typeA, typeB])` is the same as `tProp(types.or(typeA, typeB))`.

## Class model rules

- Decorate the class with `@model` and give the model type a unique application-wide name. An empty string is a valid name; in `decoratedModel`, only `undefined` means to omit registration.
- Extend `Model` with the properties that should be observable, snapshotable, and patchable.
- Access model data through either `this` or `this.$`.
- Change model data inside model actions.

:::warning No constructors
Do not declare a constructor in a class model. Use the [life-cycle hooks](#life-cycle-event-hooks) instead.
:::
...
An empty-string property name generates a setter named `set`.
...
Every class model includes a `$modelType` property:
...
There are two ways to omit `$modelType` from an input snapshot:
...
```ts
import { registerModels } from "mobx-keystone"
...
In this case, use either `customId` (in snapshots and instances) or `$modelId` (in instances only) to read or write the ID.
The ID must be unique across all model objects in the same tree, regardless of model type.
An empty-string property name (`"": idProp`) also works as an ID field, including when cloning or reconciling snapshots.
...
```ts
const myModel = new ModelWithCustomId({
...
this.customId = "my new custom id"
// or alternatively
...
```ts
customId: idProp.typedAs<`custom-${string}`>()
```
...
Always use `yield* _await(...)`. `_await` only works when delegated to with `yield*`, and a plain `yield` does not work. Both `_async` and `_await` are imported from `mobx-keystone`.
:::
...
:::warning Call `super` in life-cycle hooks
If your base model has `onInit` / `onAttachedToRootStore` and you redeclare them in your extended model, call `super.onInit(...)` / `super.onAttachedToRootStore(...)` from the extended model.
:::
...
Use `req<Req>()` as the second argument to declare that the base model must already expose a given shape before the mixin is applied.
...
`ModelData<Product>` and `ModelCreationData<Product>` reflect the exact prop types from all composed mixins.
...
type Countable = InstanceType<typeof CountableBase>
```
...
:::warning Processors must be pure
Snapshot processors must be **pure and deterministic**: they must not have
observable side effects, and the same input must always produce the same output.
They must also preserve the canonical snapshot round trip: passing the current
output snapshot through `fromSnapshotProcessor` must reproduce the same stored
model data. The library may skip the input processor when applying that already
current canonical snapshot. Do not use processors for logging, counters,
external-state reads, or other work that relies on every invocation being
observable.

This contract also applies to property snapshot processors, typed-property
