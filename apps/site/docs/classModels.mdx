...
When you need to constrain another mixin by an existing mixin result, prefer composing first and extracting the instance type:

```ts
const CountableBase = composeMixins(Entity, countableMixin)
