...
| `get(node)` | Gets the context value for a given node, recursing up the tree until a node has a set value, or the default if none is set. Usually called in actions and computed getters. |
...
| `unset(node)` | Makes the node no longer provide a context value. |
...
:::caution Don't cache `get(node)`
The value returned by `get(node)` is observable and can change, so read it where you need it instead of storing it.
:::

`apply` and `applyComputed` return the callback result unchanged. Returning a model's `$` data object does not make its model a context provider.
