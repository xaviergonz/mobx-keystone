...
In particular, `apply` is useful because it lets you pass volatile (non-property) data to your nodes at construction time, whether that happens through `new`, `fromSnapshot`, `clone`, `toTreeNode`, and so on.
For example:
