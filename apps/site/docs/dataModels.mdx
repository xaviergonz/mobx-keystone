...
Data models are defined much like class models, except that they extend `DataModel` instead of `Model`. Property defaults are applied only when wrapping a plain object with `new`, not when wrapping an existing tree node:
...
Data models follow these rules:
...
- Do not declare a constructor; use `onLazyInit` when initialization logic is needed.
