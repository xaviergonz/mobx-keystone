...
Draft data object (a copy of `originalObject`).

### `originalData: T`

Original data object (`originalObject`).
...
Paths passed to these methods follow stored tree data. For example, if an `entries` property uses `types.mapFromObject`, the path to an item's `value` is `["entries", "itemKey", "value"]`. Model IDs along that stored path are still checked.

## Example

Going back to the settings-form use case, imagine a preferences model in your app state like this:
