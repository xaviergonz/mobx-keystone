...
[**Try the live demo →**<span>Two instances of a todo app, bound to the same Loro document and synced through a `BroadcastChannel`. Edit either one and watch the other update.</span>](../examples/loroBinding/loroBinding.mdx)
...
The [Loro binding example](../examples/loroBinding/loroBinding.mdx) runs two synced instances side by side, with their full source.
