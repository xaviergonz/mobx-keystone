...
When assigning a new container to a Loro map property, populate its complete subtree before committing. The binding applies the subtree as a whole, including nested arrays, and continues synchronizing subsequent edits to those containers. Replacing a submodel with the same model ID also updates its contents.
