...
const disconnect = connectReduxDevTools(remotedev, connection, todoList)
```

Call `disconnect()` when the integration is no longer needed. It stops action logging and invokes the connection's unsubscribe callback, when provided. Calling it more than once is safe. Any retained monitor callbacks are ignored after disposal.
...
Passing `undefined` for either option retains its default value; pass `false` to disable it.
