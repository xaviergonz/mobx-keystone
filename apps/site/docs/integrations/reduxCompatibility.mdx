...
Call `disconnect()` when the integration is no longer needed. It stops action logging and invokes the connection's unsubscribe callback, when provided. Calling it more than once is safe.
