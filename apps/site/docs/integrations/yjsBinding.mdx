...
[**Try the live demo →**<span>Two instances of a todo app, bound to the same Y.js document and synced over WebRTC. Edit either one and watch the other update.</span>](../examples/yjsBinding/yjsBinding.mdx)
...
```bash
pnpm add mobx mobx-keystone yjs mobx-keystone-yjs
```
...
```bash
yarn add mobx mobx-keystone yjs mobx-keystone-yjs
```
...
:::warning
`yjsText` throws if you access it while the model is not part of a bound tree. This is due to a limitation of `Y.js`, which only allows limited manipulation of types while they are outside a `Y.Doc` tree.
:::
...
The [Y.js binding example](../examples/yjsBinding/yjsBinding.mdx) runs two synced instances side by side, with their full source.
