...
For codec-backed maps, `getOrInsert` and `getOrInsertComputed` return the stored runtime value, just like `get`. When that value is a collection, mutations through the returned value update the map's stored data.

Codec-backed sets support `union`, `intersection`, `difference`, and `symmetricDifference` with native set results and iteration order. Intersection checks membership using each collection’s own `has` method, including codec conversions. The boolean predicates `isSubsetOf`, `isSupersetOf`, and `isDisjointFrom` use each collection’s own membership rules, including codec conversions, and all seven operations work without native set-method support.

### Nested / composed codecs

A `types.refinement` wrapped around a codec checks the decoded runtime value during model construction and writes; invalid writes are rejected and rolled back.
...
When a MobX interceptor cancels an addition, update, or deletion on the backing data, the wrapper keeps its existing contents. A canceled deletion returns `false`.
...
`getOrInsert` and `getOrInsertComputed` on an `asMap` wrapper return the stored value, including any observable conversion, consistently across supported MobX versions.
