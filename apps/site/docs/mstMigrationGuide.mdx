...
1. **Runtime type checking or not**
   ...
   - The worked `mobx-keystone` examples below use `tProp(...)` because it maps more directly from MST's runtime schemas and helps with snapshot migration.
...
2. Convert properties into `tProp(...)` if you want MST-like runtime schemas during migration, or `prop(...)` if you only want TypeScript checking.
...
The same store written with both libraries:

<Tabs groupId="mst-or-keystone">
<TabItem value="mst" label="mobx-state-tree">
...
<TabItem value="keystone" label="mobx-keystone">
...
class RootStore extends Model({
  ...
  selectedTodoRef: tProp(types.maybe(types.ref(todoRef))),
}) {
  @computed
  get selectedTodo() {
...
:::note
`ExtendedModel` extends a single base class. If you need to merge more than two MST models, flatten the props or chain multiple `ExtendedModel` calls.
:::

### Action replay, serialization, patches
...
## Appendix: API mapping cheat sheet
...
| `types.late(() => T)` | Usually not needed with class references | Circular/lazy types work as plain class references; in runtime type-checking declarations use the lambda overload, e.g. `types.model<SomeModel>(() => SomeModel)`. |
...
| `types.map(T)` | `tProp(types.mapFromObject(T))`, `ObjectMap`, `asMap`, or `prop<Record<string, T>>(() => ({}))` | Prefer codecs when you want a typed `Map` runtime view over JSON-safe snapshots. Wrap with `types.skipCheck(...)` to disable runtime validation. |
| `types.Date` | `tProp(types.dateAsTimestamp)` or `tProp(types.dateAsIsoString)` | Codec types include runtime validation. Use `types.skipCheck(...)` to opt out of validation while keeping conversions. |
| `types.frozen(...)` | `frozen(data)` and/or `tProp(types.frozen(...))` | Preserve immutability + JSON-compat behavior. |
| `types.custom(...)` | `types.codec(...)` | Prefer codecs for reusable typed runtime/snapshot conversions. Use `types.skipCheck(...)` to disable validation. Use `.withTransform(...)` for prop-local transforms. |
...
15. Convert `types.Date` to `tProp(types.dateAsTimestamp)` or `tProp(types.dateAsIsoString)` when using typed runtime schemas.
...
- [ ] `types.Date` properties migrated to codecs or property transforms.
