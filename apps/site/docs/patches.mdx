...
If validation fails, the batch is rolled back. A batch is also rolled back when one of its patches throws (for example because its path does not exist), whether or not automatic type checking is enabled, so a batch is applied either completely or not at all. Errors thrown by a listener do not roll anything back: they propagate with the patches applied so far left in place, since those patches were valid. Patch and deep-change listeners still observe individual mutations; rollback emits compensating changes for mutations already published. Synchronous listener edits, including nested patch calls, join the outer validation batch. An `onSnapshot` listener may receive a restored snapshot even when its contents equal the pre-batch snapshot.
