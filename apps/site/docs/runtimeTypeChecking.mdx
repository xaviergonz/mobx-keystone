...
A type that represents an object-like map with string keys whose values all have the same type.
...
A refinement adds a custom constraint to an existing type, such as a model invariant or a numeric range.
