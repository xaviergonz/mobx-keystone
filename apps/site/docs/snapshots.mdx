...
With automatic type checking enabled, `applySnapshot` validates the completed state against the target model's type and any type constraints on its ancestors. If validation fails, the tree and its snapshots are restored to their previous state. Patch listeners may receive both the attempted changes and patches that undo them.
