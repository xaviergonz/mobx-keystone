...
- Serializing data for persistence or transport and restoring it later.
- Passing immutable views of state to code that should not depend directly on MobX observables.

When a tree node changes, a new immutable snapshot is generated for that node and each of its parents. Unchanged objects keep their previous snapshot references.
...
Snapshot generation is therefore optimized: references change only for values whose represented node or descendants actually changed.

:::caution
Do not mutate a snapshot returned by `getSnapshot`. Clone it before making changes.
:::
...
// or typed
getSnapshot<TType>(type: TType, value: TypeToData<TType>): TypeToSnapshotOut<TType>
```
...
`getSnapshot` can be used on any tree node: any model, or any plain object / array as long as it has become attached to a model or has been manually transformed into one via `toTreeNode`. It also works on primitives, in which case the primitive is returned directly.
...
fromSnapshot<TType>(
  type: TType,
  sn: TypeToSnapshotIn<TType> | TypeToSnapshotOut<TType>,
  options?: FromSnapshotOptions
): TypeToData<TType>
```

Output snapshots (e.g. the result of `getSnapshot`) are accepted as well, since they always round-trip.

Use `fromSnapshot` to restore a snapshot:
...
const bigintValue = fromSnapshot(types.bigint, "123")
const createdAtSnapshot = getSnapshot(types.dateAsTimestamp, new Date(1000))
```

If a snapshot includes `$modelType` and deserialization says the model is not in the registry, it usually means that model module was not imported at runtime (for example, import elision). In such cases either keep a runtime import (or side-effect import) or call `registerModels(...)` at startup. See [Class Models](./classModels.mdx#runtime-model-registration-registermodels).
...
If you subscribe inside an action, changes in that action are batched. The first notification's previous snapshot is the state at the end of that batch.
...
It is also possible to apply a snapshot over an object, reconciling its contents and ensuring that only the minimal set of snapshot changes / patches is triggered:
...
Properties omitted from a snapshot are removed from plain objects or reset to their model defaults when available. This also applies to names such as `toString`, `constructor`, and `__proto__`; inherited properties do not count as supplied snapshot fields.

Replacing a nested model with a plain-object snapshot without `$modelType` creates a plain object and detaches the previous model without changing its contents.
...
`applySnapshot` either applies the whole snapshot or nothing: if it throws partway (for example because of a snapshot processing error or an unregistered model type), the changes it already made are undone before the error is rethrown, keeping the same node instances. With automatic type checking enabled, it also validates the completed state against the target model's type and any type constraints on its ancestors, and a failed validation is undone the same way. Patch listeners may receive both the attempted changes and patches that undo them. Errors thrown by listeners do not undo anything, since the changes themselves were valid.
...
Like `clone`, `generateNewIds` defaults to `true`. In both helpers, passing `undefined` keeps this default; pass `false` explicitly to preserve model IDs.
