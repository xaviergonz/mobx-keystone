...
- As a way to bridge data to non-`mobx-react-lite`-enabled React components.

When a change is performed over a tree node, a new immutable snapshot of that node is generated. New immutable snapshots are also generated for all of its parents. Unchanged objects, however, keep their previous snapshot references.
...
Note: never change the contents of a snapshot object returned by `getSnapshot` directly. Clone it first.
