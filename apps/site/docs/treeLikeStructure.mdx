...
Inserting or removing array elements updates the parent paths of the following tree nodes. These index updates are batched internally; nodes that only change index keep their identity and root-store attachment. The work still grows with the number of following elements, so inserting near the beginning of a large array costs more than appending to it.
