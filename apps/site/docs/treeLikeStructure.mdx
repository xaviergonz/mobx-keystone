...
If a deep-change listener throws, the remaining global, subtree, and ancestor listeners still receive the change. The first listener error is rethrown after delivery completes; the applied mutation is not rolled back.
