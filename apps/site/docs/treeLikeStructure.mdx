...
const disposer = onDeepChange(todo, (change) => {
  ...
  if (change.type === "splice") {
    console.log(`Array at ${change.path} had ${change.removedCount} items removed and ${change.addedCount} items added at index ${change.index}`)
  }
})
```
...
- `ArraySpliceChange` - array splice with `index`, `addedCount`, `removedCount`
...
Each change includes a `path` array indicating the path from the listener target to the changed object, and a `target` reference to the actual object that changed.
