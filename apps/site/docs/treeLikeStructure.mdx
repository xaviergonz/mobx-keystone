...
Inserting or removing array elements updates the parent paths of the following tree nodes, so inserting near the beginning of a large array costs more than appending to it.

Group related edits in one `modelAction` so MobX reactions and snapshot listeners can process the completed edit together.
...
```ts
const rootStore = getRootStore<MyRootStore>(this)
if (!rootStore) {
  return undefined
}
...
Returns an observable set with all the child objects (that is, excluding primitives) of an object.
...
- `deep: boolean` (default: `false`) - `true` if the callback should be run for all children deeply or `false` if it should only run for shallow children.
- `fireForCurrentChildren: boolean` (default: `true`) - `true` if the callback should be immediately called for currently attached children, `false` if only for future attachments. Passing `undefined` keeps the default.
...
```ts
onDeepChange(target: object, listener: OnDeepChangeListener): OnDeepChangeDisposer
```

`onDeepChange` allows you to observe raw MobX change information for a tree node and all its children. This is useful when you need to detect the actual operation that occurred, such as array splices:

```ts
const disposer = onDeepChange(todo, (change) => {
  // change contains the raw change info including proper splice detection
  if (change.type === DeepChangeType.ArraySplice) {
    console.log(
      `Array at ${change.path} had ${change.removedValues.length} items removed and ${change.addedValues.length} items added at index ${change.index}`
    )
  }
})
```

The `DeepChange` type is a discriminated union of:

- `ArraySpliceChange` - array splice with `index`, `addedValues`, `removedValues`
- `ArrayUpdateChange` - direct array index assignment with `index`, `newValue`
- `ObjectAddChange` - property added with `key`, `newValue`
- `ObjectUpdateChange` - property updated with `key`, `newValue`
- `ObjectRemoveChange` - property removed with `key`

Each change includes:

- `path` - the path from the listener target to the changed object.
- `target` - the actual object that changed.
- `isInit` - whether the change happened while the tree was being initialized from defaults / snapshot data.
- `isReentrant` - whether a listener made another change while notifications were being delivered. This flag can become `true` during a callback. It is non-enumerable, so object spread and JSON serialization do not copy it.

The listener callback runs after each change passes automatic type checking and its snapshot is updated, before the outermost action has completed. Unlike `onSnapshot` or a MobX reaction, it observes individual changes within an action.

A change rejected by automatic type checking does not notify deep-change listeners. However, `applySnapshot` validates the completed state, so listeners may receive both the attempted changes and changes that undo them if validation fails.

A listener added while callbacks for the same target are running starts with the next change delivered to that target. Disposing a listener takes effect immediately: if its callback has not run yet, it will not receive the current change. Registering the same callback more than once creates separate subscriptions, each with its own disposer.

If a deep-change listener throws, the remaining global, subtree, and ancestor listeners still receive the change. After delivery completes, a single listener error is rethrown unchanged and multiple errors are grouped in a `MobxKeystoneAggregateError`; the applied mutation is not rolled back.

Patch listeners are notified after deep-change listeners. If a deep-change listener makes further edits, patches retain the order and paths of the original mutations.

### `onGlobalDeepChange`

```ts
onGlobalDeepChange(listener: (target: object, change: DeepChange) => void): OnDeepChangeDisposer
```

`onGlobalDeepChange` listens to deep changes anywhere, rather than under one specific subtree:

```ts
const disposer = onGlobalDeepChange((target, change) => {
  console.log("target", target)
  console.log("change", change)
})
```

This is mainly useful for tooling and diagnostics. In application code, prefer `onDeepChange` when you know the subtree you want to observe.
...
```ts
applyMethodCall<O extends object, K extends keyof O, FN extends O[K]>(
  node: O,
  methodName: K,
...
Allows calling a model/object/array method without the need to wrap it in `modelAction`. Unlike `runUnprotected`, this is actually an action that can be captured and replicated.
