...
test("createMonsters", () => {
  ...
  expect(monster.data.eatenHeroes && monster.data.eatenHeroes.length === 10).toBe(true)
  expect(monster.data.treasures.length === 10).toBe(true)
})
...
test("createTreasure", () => {
  ...
  expect(treasure.data.gold > 0).toBe(true)
})
