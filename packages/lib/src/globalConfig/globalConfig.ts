...
export interface GlobalConfig {
  ...
  /**
   * Avoid the `$modelType` key in snapshots generated by `getSnapshot` whenever the model
   * is typed via `types.model` (defaults to `false`).
   * Note: this setting only applies to output snapshots, not to input ones.
   */
  avoidModelTypeInTypedModelSnapshotsIfPossible: boolean
}
