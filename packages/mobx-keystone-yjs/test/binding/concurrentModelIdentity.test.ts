...
test.each(["ID", "type"])(
  ...
  (kind) => {
    ...
    applyJsonObjectToYMap(root, getSnapshot(new Root({ child: new Child({ key: "original" }) })))

    const nativeChild = root.get("child") as Y.Map<unknown>
    initialize = (value) => {
      value.child =
        kind === "type"
...
test("local writes resolve model identity conflicts introduced by transaction hooks", () => {
  @testModel("hook-identity-number")
  class NumberChild extends Model({ key: idProp, count: tProp(0) }) {}
  @testModel("hook-identity-string")
  class StringChild extends Model({ key: idProp, count: tProp("") }) {}
  @testModel("hook-identity-root")
  class Root extends Model({
    child: tProp(types.or(NumberChild, StringChild), () => new NumberChild({ key: "original" })),
  }) {}
  const doc = new Y.Doc()
  const root = doc.getMap("root")
  const binding = bindYjsToMobxKeystone({ yjsDoc: doc, yjsObject: root, mobxKeystoneType: Root })
  ...
  const before = () => {
    if (!armed) return
    armed = false
    const replacement = getSnapshot(new StringChild({ key: "replacement", count: "nine" }))
    for (const [key, value] of Object.entries(replacement)) nativeChild.set(key, value)
  }
  ...
  expect(binding.boundObject.child).toBeInstanceOf(NumberChild)
  expect(binding.boundObject.child.key).toBe("original")
  expect(binding.boundObject.child.count).toBe(2)
  expect(root.toJSON()).toEqual(getSnapshot(binding.boundObject))
})
