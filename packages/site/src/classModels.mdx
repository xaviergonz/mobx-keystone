...
- Functional models, which only define behaviors (actions/views) over plain data and _do not have_ life-cycle events (hooks).
...
Since most of the times, the only action we need for a property is a setter we can use the model option `setterAction` to reduce boilerplace and generate property setters. For example, the model above could be written as:
...
If for some reason you do not want to have/require these extra properties in your snapshots please see [functional models](./functionalModels) instead.
