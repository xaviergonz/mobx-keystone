...
Most times, the only action we need for a property is a setter we can use the prop modifier `withSetter()` to reduce boilerplace and generate property setters. For example, the model above could be written as:
