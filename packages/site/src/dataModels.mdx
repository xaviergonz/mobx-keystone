...
- They cannot have default initializers for their properties, since they use actual data objects as data.
...
Data models are defined in a similar way to class models, except that they use `DataModel` instead of `Model`. One thing to note though is that default values for properties are _not_ supported:
...
export class Todo extends DataModel({
  ...
  done: prop<boolean>(), // a required boolean
  // if you want to make an optional property then use x: prop<TYPE | undefined>()

  // if we required runtime type checking we could do this
  // text: tProp(types.string),
  // done: tProp(types.boolean),
...
```ts
const todoList: DataModelData<Todo> = [...];
...
Most times, the only action we need for a property is a setter we can use the model option `setter` to reduce boilerplace and generate property setters. For example, the model above could be written as:

```ts
@model("myCoolApp/Todo")
export class Todo extends Model({
  text: prop<string>({ setter: true }),
...
- `DataModelData<Model>` returns the data type associated to the given model. In fact it is really an alias for the one below.
- `ModelPropsData<Model>` is the type of the model props without transformations (as accessible via `model.$`).
- `ModelInstanceData<Model>` is the type of the model props with transformations (as accessible via `this`).

For example `DataModelData<Todo>` would return `{ text: string; done: boolean; }`.
...
It is possible to use a factory pattern with data models. For example:
...
@model("MyApp/Point3d")
class Point3d extends ExtendedDataModel(BasePoint, {
  z: prop<number>(),
}) {
  get sum() {
