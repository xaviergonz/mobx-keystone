...
Although `mobx-keystone` doesn't support properties which are Maps/Sets directly (for JSON compatibility purposes), you can still simulate Map/Set interfaces via the provider wrappers `arrayAsSet`, `arrayAsMap` and `objectAsMap`.
...
```ts
class ... {
  // given myObjectMap: prop<{ [k: string]: V }>(() => ({}))
  readonly myMap = objectAsMap(() => this.myObjectMap)
}
