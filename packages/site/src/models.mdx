...
class BookStore extends Model({
  ...
  // typescript version - option 1, with private method

  // note how we have to use asModelFlow to trick the TS compiler that
  // such function is awaitable
  @modelFlow
  fetchMyBooksAsync = asModelFlow(this._fetchMyBooksAsync)

  // we use function* (a function generator) where we would use`async`
  private *_fetchMyBooksAsync(token: string) {
  ...
  // typescript version - option 2, without private method

  @modelFlow
  // not how in this case we have to pass the type of this
  fetchMyBooksAsync = asModelFlow(function*(this: BookStore, token: string) {
    const myBooks = yield* myBackendClient.getBooks(token)
