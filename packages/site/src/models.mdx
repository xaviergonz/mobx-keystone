...
```ts
export interface IMyModel<TX, TY> {
...
function createModelClass<TX, TY>(modelName: string, initialX: TX, initialY: TY) {
  ...
  @model(`myApp/${modelName}`)
  class MyModel extends MyModelProps implements IMyModel<TX, TY> {
    @modelAction
    setXY(x: TX, y: TY) {
      this.x = x
      this.y = y
  ...
  return MyModel as ModelClassDeclaration<typeof MyModelProps, IMyModel<TX, TY>>
}
```
