...
export class Todo extends Model({
  ...
  // text: tcProp(types.string),
  // done: tcProp(types.boolean, false),
  // if you want to make a property truly optional then use x: tcProp(types.maybe(TYPE))
}) {
...
- `tcProp(type)` - A property of a given runtime checked type, with no default set if it is undefined in the initial data.
- `tcProp<T>(type, defaultValue: T)` - A property of a given runtime checked type, with a default set if it is undefined in the initial data. Use this only for default primitives.
- `tcProp<T>(type, defaultFn: () => T)` - A property of a given runtime checked type, with a default value generator if it is undefined in the initial data. Usually used for default objects / arrays / models.
...
@model("myApp/Auth")
class BookStore extends Model<{ books: Book[] }> {
  defaultData = {
    books: [],
  };
