...
`mobx-data-model` supports the following kinds of data:
...
class BookStore extends Model<{ books: Book[] }> {
  ...
  *fetchMyBooksAsync(token: string) {
    ...
    const myBooks: FlowRet<
      typeof myBackendClient.getBooks
    > = yield myBackendClient.getBooks(token)

    this.data.books = myBooks
  }
}
