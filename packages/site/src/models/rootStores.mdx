...
In the case of MDM, root stores are model instances from where the rest of the application state will be stored in a tree-like structure.
While it is not strictly necessary to mark these instances as root stores, doing so opens up some benefits:

1. Root stores allow the usage of the `onAttachedToRootStore(rootStore)` hook inside models.
2. Root stores can have an attached "environment", which can be shared with its whole subtree.
3. Safe referenes (more on that on the [references](../references) section) only work when used alongside root stores.
...
As a practial example, say that you have some kind of application user preferences that has to be saved to / loaded from local storage, but we also want to use the same model in a form to edit them.
...
// the old preferences object will become detached from the root store
// and therefore will stop saving changes
// at the same time the new preferences will become part of the root
// store tree, save its data and start saving any future changes
```
