...
2. Support for self-model references / cross-model references / no need for late types, no need for casting,
   etc.
3. Runtime type checking / type definitions are completely optional in `mobx-data-model`

## Typescript improvements

`mobx-state-tree` has some limitations when it comes to Typescript typings, which `mobx-data-model` tries to overcome.

### If you know Typescript you already know how to type models

`mobx-data-model` uses standard Typescript type annotations to declare the data of models, therefore lowering the learning curve a notch.
However, if you need runtime type checking, `mobx-data-model` includes a completely optional type definition / runtime type checking system.

### Self-recursive and cross-referenced models

Self-recursive or cross-referenced models are impossible (or at least very hard) to properly type in `mobx-state-tree`, but they become trivial with `mobx-data-model`.
...
In `mobx-data-model` snapshots are usually only expected when dealing with `getSnapshot` and `fromSnapshot`, so it leads to a simpler usage:
