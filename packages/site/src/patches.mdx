...
As seen in the previous [snapshots](../snapshots) section, any change made to a tree node will generate a new snapshot, but this is only one of the three possible ways `mobx-keystone` offers to detect changes. The second way are patches.
