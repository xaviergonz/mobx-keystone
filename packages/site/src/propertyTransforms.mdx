...
Property transforms can also be used standalone:
...
which is useful when creating the object anew:

```ts
const myModel = new MyModel({
  timestamp: asDate.dataToProp(new Date()),
})
```
...
That being said, check the new `prop_dateString`, `tProp_dateString`, `prop_dateTimestamp` and `tProp_dateTimestamp` described in the [Map, Sets, Dates](./mapsSetsDates) section. It should be more straightforward.
