...
class TodoList extends Model<{ list: Todo[]; selected?: Ref<Todo> }> {
  ...
  selectTodo(todo: Todo | undefined) {
    if (!this.data.list.includes(todo)) {
      throw new Error("cannot select a todo that is not in the list")
    }
    this.data.selected = todo ? ref(todo) : undefined
  }
}
```
...
class TodoList extends Model<{ list: Todo[]; selected?: Ref<Todo> }> {
  ...
  selectTodo(todo: Todo | undefined) {
    ...
    this.data.selected = todo ? ref(todo, { autoDetach: true }) : undefined
  }
}
...
- Model/object property: the property will become undefined (therefore remember to type the property as optional in Typescript).
