...
Although usually contexts are preferred for this case (see the [contexts](./contexts) section), root stores can be also an alternative to store global contextual/environmental runtime data that doesn't really need to be serialized anywhere yet does need to be shared across the whole application.
