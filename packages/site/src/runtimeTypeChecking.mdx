...
- Typechecking in `mobx-keystone` is performed over instances once they have been created, not over snapshots, so the type definitions should be based on that fact.
- There is no `types.optional` since setting default values is already covered by the `prop` default values.
