...
Snapshots in `mobx-data-model` mainly serve these two purposes:
...
This means, as mentioned beofre, that snapshots generation is automatically optimized to only change their reference when the object they represent (and their children) actually change.

## Getting the snapshot of an instance

Getting the snapshot out of a model or any of its children is as easy as this:
...
// this returns an object like
{
  ...
  "$$metadata": {
    "type": "myApp/Todo",
    "id": "_SvYKK1zWYpBj7ST_fVV2"
  }
}
```

The additional `$$metadata` sub-object is used to:

- Allow `fromSnapshot` to recognize the original class and faithfully recreate it, rather than assume it is a plain object.
- Improve snapshot reconciliation.
- Allow references to work (more on this on the [references](./references) section).

This metadata is only required for models, this is, arrays, plain objects and primitives don't have this extra field.
...
`fromSnapshot` by default will generate new ids for each model, as well as fix any references automatically as long as they are inside the same snapshot. If you wish to keep the same ids you can pass an extra option object as shown:

```ts
const todo = fromSnapshot<Todo>(todoSnapshot, { generateNewIds: false })
```
...
Since that is a very common pattern, `mobx-data-model` offers an `onSnapshot` function that will call a listener with the new snapshot and the previous snapshot everytime it changes.
