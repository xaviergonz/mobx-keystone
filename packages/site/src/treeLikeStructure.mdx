...
5. Note that class models with the `valueType: true` option will get cloned automatically before getting inserted as a child of another node so, for all practical purposes, rule 3 does not apply and act more akin to a primitive.
...
To check if an non-primitive has been turned into a tree node you can use `isTreeNode(value: object): boolean`, or
...
### `findChildren<T extends object = any>(root: object, predicate: (node: object) => boolean): ReadonlySet<T>`
...
Besides the aforementioned `isTreeNode`, `assertIsTreeNode` and `toTreeNode` methods, there's also the `detach(value: object)` method, which allows a node to get detached from its parent following this logic:
